...
AI 能力只有在模型注册、凭证引用、Prompt 版本、工具注册、MCP、Run/Step/ToolCall、知识生命周期、用量审计、评测和治理共同达到标准能力剖面后才能发布。AI 模块不是聊天页面，也不直接读取其它模块的 Mapper。
...
1. 配置两个通过相同 Provider 契约测试的独立适配器，领域层不感知其协议和供应商。
2. 发布一个版本化 Prompt。
3. 注册一个只读平台工具和一个 MCP 工具。
4. 创建 Run，记录模型调用、工具参数、权限结果、Token、费用和延迟。
5. 建立具备导入、版本、解析、切片、索引、权限过滤、引用、重建和删除生命周期的知识库。
